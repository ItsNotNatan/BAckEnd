...
const criarMaterial = async (solicitante, itens, anexos) => {
  ...
  // 🚀 AQUI OCORREU A ATUALIZAÇÃO PRINCIPAL PARA A CRIAÇÃO DE MATERIAL
  const itensDB = itens.map(i => ({
    ...
    fornecedor: i.fornecedor || null,            // 👈 Novo campo mapeado do front-end
    referencia: i.referencia || null,            // 👈 Novo campo mapeado do front-end
    wbs_element: i.wbs || null,                  // 👈 Novo campo mapeado do front-end
    alocacao: i.alocacao || null                 // 👈 Novo campo mapeado do front-end
  }));
...
const atualizarItensDaSolicitacao = async (solicitacaoId, itens) => {
  ...
  // 🚀 AQUI TAMBÉM ATUALIZAMOS PARA NÃO PERDERMOS OS DADOS NA EDIÇÃO INLINE
  const itensDB = itens.map(i => {
    let precoLimpo = 0;
    if (i.poNetPrice) {
      let v = String(i.poNetPrice).replace(/[^\d.,-]/g, '');
    ...
    return {
      solicitacao_id: solicitacaoId,
      desenho_sap_manual: i.desenhoSAP || i.desenho_sap_manual || null,                       // 👈 Adicionado suporte para edição
      part_number_manual: i.numPecaFabricante || i.part_number_manual || i.part_number || null,
      fornecedor: i.fornecedor || null,
      referencia: i.referencia || null,                                                       // 👈 Adicionado
      ...
      wbs_element: i.wbsElement || i.wbs_element || i.wbs || null,                            // 👈 Ajustado
